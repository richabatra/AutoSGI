################################################################################
# ADNI LIPIDOMICS - AUTOSGI CASE STUDIES (2 & 3)
################################################################################
#
...
################################################################################
# CASE STUDY 3: PATHWAY-LEVEL SELECTION AUTOSGI
################################################################################
#
# Approach: Directly clusters lipid pathways based on their abundance patterns
#           without transformation. This provides a complementary view to 
#           enrichment and pseudotime approaches.
#
################################################################################

## Prepare lipid abundance matrix ----

# Extract lipid abundance data (samples × lipids)
lipid_abundance_matrix <- lipid_se_baseline %>% assay() %>% t() %>% data.frame()

# Ensure rownames match between data and phenotypes
rownames(lipid_abundance_matrix) <- rownames(clinical_phenotypes)


## Initialize AutoSGI parameters for pathway selection ----

pathway_params <- sgi_params_init(
  dataset = lipid_abundance_matrix, 
  clins = clinical_phenotypes, 
  minsize = (nrow(lipid_abundance_matrix) / 20), 
...
# Get lipid class annotations from row metadata
pathway_annotations <- lipid_se_baseline %>% rowData() %>% data.frame()


## Run pathway-based set selection ----

# Execute set selection using lipid class groupings
...
#
# Summary of outputs:
#   - ssGSEA results: Enrichment-based clustering of lipid classes
#   - DPT results: Pseudotime-based clustering revealing temporal patterns
#   - Pathway results: Direct pathway-level clustering
#
# Each analysis produces:
#   - Hierarchical selection plots (PDF)
#   - Summary visualizations (PDF, where applicable)
#   - Cluster assignments (Excel)
#   - Association results (Excel)
#
################################################################################
